...
nDetDynamicMaterial::nDetDynamicMaterial() : name(), init(false), debug(false), needsConverted(false), needsScaled(false),
                                             density(0), convFactor(1), scalingFactor(1), nElements(0), valuesToRead(0), 
                                             material(NULL), mpt(NULL), currArray(NULL) { 
...
bool nDetDynamicMaterial::decode(const std::vector<std::string> &args){
	...
	if(nArgs < nArgsExpected){ // Check the number of arguments
		std::cout << Display::ErrorStr("nDetDynamicMaterial") << "Invalid number of arguments given to " << cmd << "! Expected " << nArgsExpected << " but received " << nArgs << ".\n" << Display::ResetStr();
		return false;
	}

	if(cmd == "setName"){ // setName <name>
		name = args.at(1);
		if(debug)
		...
		if(nArgs >= 4){ // User specified a unit, we need to do a unit conversion
			...
			needsConverted = true;
			if(debug)
				std::cout << " debug: Added array \"" << args.at(1) << "\" with " << variables[args.at(1)].size() << " elements (units=" << args.at(3) << ", factor=" << convFactor << ")\n";
		}
		else{ // No unit specified, use the value as-is
			needsConverted = false;
			if(debug)
				std::cout << " debug: Added array \"" << args.at(1) << "\" with " << variables[args.at(1)].size() << " elements\n";
		}
